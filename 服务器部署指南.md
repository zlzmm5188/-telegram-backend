...
由于安全原因，我无法直接连接到您的服务器，但我已经为您准备了完整的部署包。

## 🎯 方案1：一键部署脚本

### 步骤1：连接到服务器
```bash
ssh root@31.97.122.31
# 输入密码：Qq05582265188+
```
...
if [ -z "$SERVER_IP" ]; then
    SERVER_IP=$(curl -s ifconfig.me 2>/dev/null || echo "31.97.122.31")
fi
...
# 5. 创建Web客户端
echo "🌐 创建前端文件..."
cat > telegram_web_client.html << 'FRONTEND_EOF'
<!DOCTYPE html>
<html lang="zh-CN">
<head>
    <meta charset="UTF-8">
...
<body>
        ...
        <div class="main-content">
            ...
            <div class="chat-area">
                <div class="messages" id="messages">
                    <div class="message">
                        <strong>📱 手机版Telegram客户端</strong><br>
                        ✅ 支持多会话管理<br>
                        ✅ 避免账号掉号<br>
                        ✅ 响应式设计<br><br>
    ...
    <script>
        ...
        function addMessage(content, type = 'system') {
            ...
            messageDiv.innerHTML = `<div style="font-size: 12px; color: #666;">${new Date().toLocaleTimeString()}</div><div>${content}</div>`;
        ...
        document.getElementById('checkStatusBtn').addEventListener('click', async () => {
            try {
                ...
                if (result.authorized) {
                    ...
                    addMessage(`✅ 连接成功: ${result.user.first_name} (@${result.user.username})`);
                } else {
                    document.getElementById('status').innerHTML = `状态: ❌ 未授权 - ${result.reason}`;
                    addMessage(`❌ 未授权: ${result.reason}`);
                }
            } catch (error) {
        ...
        document.getElementById('getUserInfoBtn').addEventListener('click', async () => {
            try {
                ...
                if (result.id) {
                    ...
                    addMessage(`❌ 获取用户信息失败: ${result.error}`);
                }
            } catch (error) {
...
<body>
    <div class="container">
        ...
        <div>
            <a href="telegram_web_client.html" class="btn">🚀 开始使用</a>
        </div>
        
        <div style="margin-top: 30px; font-size: 14px; color: #666;">
            <p>API状态: <a href="/debug/session" target="_blank">检查后端</a></p>
            <p>技术支持: Telegram多会话管理系统</p>
        </div>
    </div>
</body>
</html>
...
# 7. 配置Nginx
echo "🌐 配置Nginx..."
cat > /etc/nginx/sites-available/telegram-client << NGINX_EOF
server {
    listen 80;
    server_name _;
    
    root /root/telegram-client;
    index index.html;
...
ln -sf /etc/nginx/sites-available/telegram-client /etc/nginx/sites-enabled/
...
echo "   客户端: http://$SERVER_IP/telegram_web_client.html"
echo ""
echo "🔧 管理命令："
...
echo ""
echo "📋 使用说明："
echo "1. 在手机浏览器中访问: http://$SERVER_IP"
echo "2. 点击'开始使用'进入客户端"
echo "3. 输入子域名（如：user1, user2）"
echo "4. 完成Telegram登录验证"
echo "5. 开始使用多会话Telegram客户端"
echo ""
echo "🔐 重要提醒："
echo "- 每个子域名对应独立的Telegram会话"
echo "- 支持同时登录多个账号而不会掉号"
echo "- 所有数据都安全存储在您的服务器上"
echo "=========================================="
EOF
...
pip3 install --break-system-packages fastapi uvicorn telethon

# 2. 创建目录
mkdir -p ~/telegram-client
cd ~/telegram-client

# 3. 创建后端文件（将之前的main.py内容复制过来）
# 4. 创建前端文件（将HTML内容复制过来）
# 5. 配置Nginx
# 6. 启动服务
```
...
1. **打开手机浏览器**
2. **访问**: `http://31.97.122.31`
3. **点击"开始使用"**
4. **输入子域名**: 例如 `mobile1`
5. **输入手机号**: `+8613800138000`
6. **点击"发送验证码"**
7. **输入收到的验证码**
8. **点击"验证登录"**
9. **开始使用客户端**
...
# 检查服务状态
systemctl status nginx
tail -f ~/telegram-client/api.log

# 重启服务
systemctl restart nginx
cd ~/telegram-client && pkill -f uvicorn
nohup python3 -m uvicorn main:app --host 0.0.0.0 --port 8000 > api.log 2>&1 &
```
